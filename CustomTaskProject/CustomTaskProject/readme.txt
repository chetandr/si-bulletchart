...
3. The output of the custom task is of type list. E.g. If the task is to extract all first_names (either male or female) found in the text,
   then all extracted first_names should be added to the returned list. And that list will be available in pig the context with the name
   given in the 'output' field.
   
   Let's consider example below. This is a sample task definition from SI flow file.
	
	  split_YearFromMovies :
	      type : map
	      operator : custom
	      class : FNameExtractor
	      transform : blog_content
	      output : first_names
	      parameter : 'F'
	  
   In this example, 'blog_content' is passed as input to the task. The type of names (Male/Female) to be extracted is controlled by parameter (M/F). 
   And the returned list from the task is available with name 'first_names'. Class 'FNameExtractor' should be present under package 'in.co.psl.si.udf'.
...
4. It returns count of all words in a movie name that start with letter 'T'.
